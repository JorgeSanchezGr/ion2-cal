...
var CalendarModal = /** @class */ (function () {
    ...
    CalendarModal.prototype.init = function () {
        ...
        if (this.showYearPicker) {
            this.createYearPicker(this.showYearPicker.start, this.showYearPicker.end);
        }
        else {
        ...
        switch (this._d.pickMode) {
            ...
            case "multi":
                if (this._d.defaultDates && this._d.defaultDates.length) {
                    this.datesTemp = this._d.defaultDates.map(function (e) {
    ...
    CalendarModal.prototype.createYearPicker = function (startTime, endTime) {
        ...
        this.years.reverse();
        this.year = new Date(this.options.start).getFullYear();
    };
    CalendarModal.prototype.nextMonth = function (infiniteScroll) {
        ...
        var finalMoment = moment(final.original.time);
        if (!this.showYearPicker ||
            (this.showYearPicker &&
                (finalMoment.year() < this.showYearPicker.end ||
                    (finalMoment.year() == this.showYearPicker.end &&
                        finalMoment.month() < 11)))) {
            var nextTime = finalMoment.add(1, "M").valueOf();
    ...
    CalendarModal.prototype.backwardsMonth = function () {
        if (!this.stop) {
            ...
            var firstMoment = moment(first.original.time);
            if (!this.showYearPicker ||
                (this.showYearPicker &&
                    (this.showYearPicker.start < firstMoment.year() ||
                        (this.showYearPicker.start == firstMoment.year() &&
                            firstMoment.month() > 2)))) {
                var firstTime = firstMoment.subtract(1, "M").valueOf();
    ...
    CalendarModal.prototype.changedYearSelection = function () {
        ...
        var first = new Date(this.year, 1, 2);
        // let last = new Date(this.year, 12, 0)
        var months = this.findInitMonthNumber(first) + this.countNextMonths;
        console.log(first);
        this.calendarMonths = this.calSvc.createMonthsByPeriod(first.getTime(), months <= 1 ? 3 : months, this._d);
        setTimeout(function () {
