...
export class CalendarModal {
  ...
  nextMonth(infiniteScroll: InfiniteScroll) {
    ...
    let finalMoment = moment(final.original.time)
    ...
    if (
      ...
      let nextTime = finalMoment.add(1, "M").valueOf()
  ...
  backwardsMonth() {
    if (!this.stop) {
      ...
      let firstMoment = moment(first.original.time)
      ...
      if (
        ...
        let firstTime = firstMoment.subtract(1, "M").valueOf()
  ...
  changedYearSelection() {
    ...
    let first = new Date(this.year, 1, 2)
    // let last = new Date(this.year, 12, 0)
    let months = this.findInitMonthNumber(first) + this.countNextMonths
    console.log(first)
    this.calendarMonths = this.calSvc.createMonthsByPeriod(
      first.getTime(),
      months <= 1 ? 3 : months,
